...
const login = async (req, res) => {
  ...
  if (!isPasswordCorrect) {
    ...
    if (user.blocked_until_password && user.blocked_until_password > new Date()) {
      const remaininMinutes = Math.ceil( 
        (user.blocked_until_password - new Date() ) / (60 * 1000)
      );
      message =  `Your account is block for passwor. try again after ${user.blocked_until_password}`;
    } else {
      const attemptsRemaining = 4 - user.wrong_password_attempts;
  ...
  res.status(StatusCodes.OK).json({
    user: {name: user.name, email: user.email, phone_exist, userId: user.id, login_pin_exist},
