...
const setLoginPinFirst = async (req, res) => {
  ...
  const hashPin = await bcrypt.hash(login_pin, salt);
...
const verifyPin = async (req, res) => {
  ...
  const user = await User.findById(userId);
  if (!user) {
    throw new NotFoundError(`Nouser with id : ${userId}`);
  }
  ...
  if (!isVerifyingPin) {
    ...
    if (user.blocked_until_pin && user.blocked_until_pin > new Date()) {
      const blockedTime = Math.ceil((user.blocked_until_pin - new Date()) /60000);
      message = `Pls try again after ${blockedTime} minutes.`;
    } else {
      const attempsRemaining = 3 - user.wrong_pin_attempts;
      message = attempsRemaining > 0 ? `Wrong PIN. ${attempsRemaining} attempt remaining.` : 'You have been blocked due to multiple wrong attemps. Please try again after 15 minutes.';
    }
