...
const generateStockData = async(symbol) => {
  ...
  if (patternType < 0.15) {
    ...
    if (Math.random() < 0.5) {
      high = close + Math.random() * 4; // Lond=g Bullish candle
  ...
  if (!lastItem || now-new Date(lastItem.timestamp) > 1*60*1000) {      
    //1 minute interval
    stock.dayTimeSeries.push({ timestamp, time, _internal_originalTime: time, open: roundToTwoDecimals(currentPrice), high, low, close });
...
const store10Min = async (symbol) => {
  ...
  stock.tenMinTimeSeries.push({
    ...
    close: roundToTwoDecimals(latestItem.close ?? currentPrice),
  });
